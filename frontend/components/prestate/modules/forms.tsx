...
import type { Device, FieldLogicOp, FormLeadField, LandingPageData, SiteConfig, FormThankYouPage } from "@/lib/prestate/types";
...
const FIELD_TYPES: { type: FieldType; label: string; icon: React.ReactNode }[] = [
  ...
  { type: "textarea", label: "Message box", icon: <MessageSquare size={14} /> },
...
export function FormsModule({
  ...
  const [activeTab, setActiveTab] = useState<"fields" | "delivery" | "brochure" | "library" | "embed">("fields");
  ...
  const patchPdf = (patch: Partial<SiteConfig["form"]["pdf"]>) =>
    onPatch((c) => ({ ...c, form: { ...c.form, pdf: { ...c.form.pdf, ...patch } } }));
  ...
  const addField = (type: FieldType) => {
    ...
    const newFld: FormLeadField = {
      ...
      placeholder: type === "select" || type === "radio" ? "Select an option" : type === "phone" ? "+91 98765 43210" : type === "email" ? "name@email.com" : "",
      required: type === "phone" || type === "text",
      options: type === "select" || type === "radio" ? ["2 BHK Luxury", "3 BHK Premium", "4 BHK Sky Villa"] : undefined,
    };
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "var(--ps-bg)", color: "var(--ps-ink)", overflow: "hidden", ...siteThemeStyle(cfg.brand) }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <span style={{ fontSize: 13.5, fontWeight: 800, color: "#fff", display: "flex", alignItems: "center", gap: 7 }}>
            <FileText size={16} style={{ color: "var(--ps-primary)" }} /> Universal Form Builder
          </span>
        ...
        <div style={{ display: "inline-flex", background: "rgba(0, 0, 0, 0.35)", borderRadius: 10, padding: 3, border: "1px solid var(--ps-line-strong)" }}>
          {[
            { key: "fields", label: "Fields & Structure", icon: TextCursorInput },
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
        ...
        >
          {activeTab === "fields" ? (
            <>
              ...
              <div style={{ background: "var(--ps-panel-raised)", border: "1px solid var(--ps-line-strong)", borderRadius: 14, padding: "16px 16px" }}>
                <div style={{ fontSize: 13, fontWeight: 800, color: "#fff", marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>
                  <TextCursorInput size={16} style={{ color: "var(--ps-primary)" }} /> Form Heading & Button
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 700, color: "var(--ps-muted)", display: "block", marginBottom: 4 }}>Form Header Title</label>
                    <input
                      className="ps-input"
                      value={form.name ?? ""}
                  ...
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 700, color: "var(--ps-muted)", display: "block", marginBottom: 4 }}>Form Sub-Description</label>
                    <input
                      className="ps-input"
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    <div>
                      <label style={{ fontSize: 11, fontWeight: 700, color: "var(--ps-muted)", display: "block", marginBottom: 4 }}>Submit Button Text</label>
                      <input
                        className="ps-input"
                    ...
                    <div>
                      <label style={{ fontSize: 11, fontWeight: 700, color: "var(--ps-muted)", display: "block", marginBottom: 4 }}>Multi-Step Wizard</label>
                      <div style={{ display: "flex", alignItems: "center", height: 34 }}>
                        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "#fff", cursor: "pointer" }}>
                          <input
                            ...
                            onChange={(e) => patchForm({ multiStep: e.target.checked })}
              ...
              <div style={{ background: "var(--ps-panel-raised)", border: "1px solid var(--ps-line-strong)", borderRadius: 14, padding: "16px 16px" }}>
                <div style={{ fontSize: 13, fontWeight: 800, color: "#fff", marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}>
                  <Plus size={16} style={{ color: "var(--ps-primary)" }} /> Add Input Field
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 6 }}>
                  {FIELD_TYPES.map((ft) => (
                    ...
                    >
                      <span style={{ color: "var(--ps-primary)" }}>{ft.icon}</span>
                      <span>{ft.label}</span>
                    </button>
                  ))}
                </div>
              ...
              <div style={{ background: "var(--ps-panel-raised)", border: "1px solid var(--ps-line-strong)", borderRadius: 14, padding: "16px 16px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                  <div style={{ fontSize: 13, fontWeight: 800, color: "#fff", display: "flex", alignItems: "center", gap: 6 }}>
                    <SlidersHorizontal size={16} style={{ color: "var(--ps-primary)" }} /> Form Fields ({fields.length})
                  </div>
                  <span style={{ fontSize: 11, color: "var(--ps-muted)" }}>Drag or select to configure</span>
                </div>
                  ...
                  {fields.map((f, i) => {
                    ...
                    return (
                      <div
                        ...
                        style={{
                          background: isSelected ? "rgba(99, 102, 241, 0.18)" : "var(--ps-bg)",
                          border: isSelected ? "2px solid var(--ps-primary)" : "1px solid var(--ps-line)",
                          borderRadius: 10,
                          padding: "10px 12px",
                          cursor: "pointer",
                          transition: "all 0.12s",
                      ...
                      >
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <GripVertical size={14} style={{ color: "var(--ps-muted)", flexShrink: 0 }} />
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: 12.5, fontWeight: 800, color: isSelected ? "#818cf8" : "#fff", display: "flex", alignItems: "center", gap: 6 }}>
                              {f.label || "Untitled Field"}
                              {f.required ? <span style={{ color: "var(--ps-danger)", fontSize: 11 }}>*</span> : null}
                            </div>
                            <div style={{ fontSize: 10.5, color: "var(--ps-muted)", textTransform: "uppercase", marginTop: 2 }}>{f.type}</div>
                          </div>
                          <div style={{ display: "flex", gap: 2 }}>
                            <button
                              ...
                              style={{ background: "none", border: "none", color: "var(--ps-muted)", cursor: i === 0 ? "default" : "pointer", padding: 3, opacity: i === 0 ? 0.3 : 1 }}
                            >
                              <ChevronUp size={14} />
                            </button>
                            <button
                              ...
                              onClick={(e) => { e.stopPropagation(); moveField(f.id, 1); }}
                              style={{ background: "none", border: "none", color: "var(--ps-muted)", cursor: i === fields.length - 1 ? "default" : "pointer", padding: 3, opacity: i === fields.length - 1 ? 0.3 : 1 }}
                            >
                              <ChevronDown size={14} />
                            </button>
                            <button
                              ...
                              style={{ background: "none", border: "none", color: "var(--ps-muted)", cursor: "pointer", padding: 3 }}
                            >
                              <Trash2 size={13} />
                            </button>
                          </div>
                        ...
                        {isSelected ? (
                          <div style={{ marginTop: 12, paddingTop: 10, borderTop: "1px solid var(--ps-line)", display: "flex", flexDirection: "column", gap: 8 }}>
                            <div>
                              <label style={{ fontSize: 10.5, fontWeight: 700, color: "var(--ps-muted)" }}>Field Label</label>
                              <input
                                className="ps-input"
                                value={f.label ?? ""}
                                onChange={(e) => patchField(f.id, { label: e.target.value })}
                                style={{ width: "100%", fontSize: 12, marginTop: 2, background: "var(--ps-panel-raised)", color: "#fff" }}
                              />
                            </div>
                            <div>
                              <label style={{ fontSize: 10.5, fontWeight: 700, color: "var(--ps-muted)" }}>Placeholder Text</label>
                              <input
                                className="ps-input"
                                value={f.placeholder ?? ""}
                            ...
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 4 }}>
                              <span style={{ fontSize: 11.5, fontWeight: 600, color: "var(--ps-slate)" }}>Required Input</span>
                              <input
                                type="checkbox"
            ...
            <>
              ...
              <div style={{ background: "var(--ps-panel-raised)", border: "1px solid var(--ps-line-strong)", borderRadius: 14, padding: "16px 16px" }}>
                <div style={{ fontSize: 13, fontWeight: 800, color: "#fff", marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>
                  <Bell size={16} style={{ color: "var(--ps-primary)" }} /> Lead Routing & Alerts
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 700, color: "var(--ps-muted)", display: "block", marginBottom: 4 }}>Lead Notification Email</label>
                    <input
                      className="ps-input"
                  ...
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 700, color: "var(--ps-muted)", display: "block", marginBottom: 4 }}>Instant WhatsApp Lead Alert Number</label>
                    <input
                      className="ps-input"
                      value={form.whatsapp ?? ""}
            ...
            <>
              ...
              <div style={{ background: "var(--ps-panel-raised)", border: "1px solid var(--ps-line-strong)", borderRadius: 14, padding: "16px 16px" }}>
                <div style={{ fontSize: 13, fontWeight: 800, color: "#fff", marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>
                  <FileDown size={16} style={{ color: "var(--ps-primary)" }} /> Instant Brochure PDF Gate
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 700, color: "var(--ps-muted)", display: "block", marginBottom: 4 }}>Upload Project Brochure (PDF)</label>
                    <input
                      ref={pdfInputRef}
                      type="file"
                      accept=".pdf"
                  ...
                  {form.pdf?.filename ? (
                    <div style={{ padding: "8px 12px", borderRadius: 8, background: "rgba(52, 211, 153, 0.15)", border: "1px solid rgba(52, 211, 153, 0.3)", color: "var(--ps-success)", fontSize: 12, fontWeight: 700 }}>
            ...
            <>
              ...
              <div style={{ background: "var(--ps-panel-raised)", border: "1px solid var(--ps-line-strong)", borderRadius: 14, padding: "16px 16px" }}>
                <div style={{ fontSize: 13, fontWeight: 800, color: "#fff", marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}>
                  <LayoutTemplate size={16} style={{ color: "var(--ps-primary)" }} /> Reusable Form Templates ({library.length})
                </div>
                <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                  <input
                    ...
                    style={{ flex: 1, fontSize: 12, background: "var(--ps-bg)", color: "#fff" }}
                  />
                  <button
                    type="button"
                    onClick={createNewForm}
                    style={{ background: "var(--ps-primary)", border: "none", color: "#fff", padding: "0 12px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer" }}
                  >
                    Create
                  </button>
                </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {library.map((item) => (
                    <div key={item.id} style={{ background: "var(--ps-bg)", border: "1px solid var(--ps-line)", borderRadius: 10, padding: "10px 12px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      <div>
                        <div style={{ fontSize: 12.5, fontWeight: 800, color: "#fff" }}>{item.name}</div>
                        <div style={{ fontSize: 11, color: "var(--ps-muted)" }}>{item.fields.length} input fields</div>
                      </div>
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        style={{ background: "rgba(99, 102, 241, 0.2)", border: "1px solid var(--ps-primary)", color: "#818cf8", padding: "4px 10px", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer" }}
                      >
                        Load
                      </button>
                    </div>
            ...
            <>
              ...
              <div style={{ background: "var(--ps-panel-raised)", border: "1px solid var(--ps-line-strong)", borderRadius: 14, padding: "16px 16px" }}>
                <div style={{ fontSize: 13, fontWeight: 800, color: "#fff", marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>
                  <Code2 size={16} style={{ color: "var(--ps-primary)" }} /> Universal Embed Snippet
                </div>
                <p style={{ fontSize: 11.5, color: "var(--ps-muted)", margin: "0 0 10px", lineHeight: 1.45 }}>
                  Embed this form on external landing pages, WordPress, Webflow, or custom domains.
                </p>
                <div style={{ background: "var(--ps-bg)", padding: 12, borderRadius: 8, border: "1px solid var(--ps-line)", fontSize: 11.5, fontFamily: "monospace", color: "#a5b4fc", wordBreak: "break-all", marginBottom: 10 }}>
                  {embedCode}
                </div>
                <button
                  type="button"
                  onClick={() => copyText(embedCode, "Embed code copied to clipboard")}
                  style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, background: "var(--ps-primary)", border: "none", color: "#fff", padding: "9px 14px", borderRadius: 8, fontSize: 12.5, fontWeight: 700, cursor: "pointer" }}
                >
        ...
        >
          ...
          <div
            style={{
              width: device === "desktop" ? 560 : device === "tablet" ? 480 : 380,
              maxWidth: "100%",
              background: "#fff",
          ...
          >
            ...
            {/* Live Form Content */}
            <div style={{ padding: device === "mobile" ? "26px 20px" : "36px 32px" }}>
              {previewSubmitted ? (
                <div style={{ textAlign: "center", padding: "30px 10px" }}>
                  <div style={{ width: 52, height: 52, borderRadius: "50%", background: "rgba(52, 211, 153, 0.15)", color: "#10b981", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
                    <Check size={26} />
                  </div>
                  <h3 style={{ fontSize: 20, fontWeight: 800, color: "#0f172a", margin: "0 0 8px" }}>Enquiry Submitted Successfully!</h3>
                  <p style={{ fontSize: 13, color: "#64748b", margin: "0 0 20px" }}>
                    Thank you for your interest. Our representative will contact you shortly with the official brochure and pricing.
                  </p>
                  <button
                    type="button"
                    onClick={() => setPreviewSubmitted(false)}
                    style={{ background: "var(--ps-primary)", color: "#fff", border: "none", padding: "8px 18px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer" }}
                  >
                    ← Test Again
                  </button>
                </div>
                ...
                >
                  <div>
                    ...
                    {form.description ? (
                      <p style={{ fontSize: 12.5, color: "#64748b", margin: 0, lineHeight: 1.5 }}>{form.description}</p>
                    ) : null}
                  </div>

                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    {fields.length === 0 ? (
                      <div style={{ padding: "30px 10px", textAlign: "center", color: "#94a3b8", fontSize: 13, border: "1.5px dashed #cbd5e1", borderRadius: 10 }}>
                        No fields added yet. Click &quot;Add Input Field&quot; in the left sidebar.
                      </div>
                    ) : null}
                    ...
                    {fields.map((f) => (
                      <div key={f.id} style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                        ...
                        {f.type === "textarea" ? (
                          <textarea
                            placeholder={f.placeholder || "Enter your message..."}
                            rows={3}
                            style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1px solid #cbd5e1", fontSize: 13, color: "#0f172a", outline: "none", resize: "none" }}
                          />
                        ) : f.type === "select" ? (
                          <select style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1px solid #cbd5e1", fontSize: 13, color: "#0f172a", outline: "none" }}>
                          ...
                          <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, color: "#475569", cursor: "pointer" }}>
                            ...
                            <span>I agree to receive project updates & official brochure via WhatsApp/Email</span>
                          </label>
                        ) : (
                          <input
                            type={f.type === "email" ? "email" : f.type === "number" ? "number" : "text"}
                            placeholder={f.placeholder || `Enter ${f.label.toLowerCase()}`}
                            style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1px solid #cbd5e1", fontSize: 13, color: "#0f172a", outline: "none" }}
                          />
                        )}
                      </div>
                    ))}
